...
void resetMaze()
{
    //Used to reset all variables to their original values
    enemyAStanding = "true";
    enemyBStanding = "true";
    enemyCStanding = "true";
    redFalconCommanderStanding = "true";
    redFalconCommanderActive = "false";
    playerBulletCount = 0;
    enemyABulletCount = 0;
    enemyBBulletCount = 0;
    enemyCBulletCount = 0;
    redFalconCommanderBulletCount = 0;
    playerHealth = 100;
    enemyAHealth = 50;
    enemyBHealth = 50;
    enemyCHealth = 50;
    redFalconCommanderHealth = 500;
    remainingLives = 5;
    ...
    enemyCX = 115;
    enemyCY = 40;
    redFalconCommanderX = 105;
    redFalconCommanderY = 20;
    enemyADirection = "Up";
...
void loadMazeFromFile()
{
    //Used to load all values of variables in text files
    string line;
    fstream file;
    file.open("Maze.txt", ios::in);
    while (getline(file, line))
    {
        enemyAStanding = parsing(line, 1);
        enemyBStanding = parsing(line, 2);
        enemyCStanding = parsing(line, 3);
        redFalconCommanderStanding = parsing(line, 4);
        redFalconCommanderActive = parsing(line, 5);
        getline(file, line);
        playerHealth = stoi(parsing(line, 1));
        enemyAHealth = stoi(parsing(line, 2));
        enemyBHealth = stoi(parsing(line, 3));
        enemyCHealth = stoi(parsing(line, 4));
        redFalconCommanderHealth = stoi(parsing(line, 5));
        getline(file, line);
        remainingLives = stoi(parsing(line, 1));
        bulletsRemaining = stoi(parsing(line, 2));
        getline(file, line);
        mainPlayerX = stoi(parsing(line, 1));
        mainPlayerY = stoi(parsing(line, 2));
        enemyAX = stoi(parsing(line, 3));
        enemyAY = stoi(parsing(line, 4));
        enemyBX = stoi(parsing(line, 5));
        enemyBY = stoi(parsing(line, 6));
        enemyCX = stoi(parsing(line, 7));
        enemyCY = stoi(parsing(line, 8));
        redFalconCommanderX = stoi(parsing(line, 9));
        redFalconCommanderY = stoi(parsing(line, 10));
        getline(file, line);
        enemyADirection = parsing(line, 1);
        enemyBDirection = parsing(line, 2);
        enemyCDirection = parsing(line, 3);
        redFalconCommanderDirection = parsing(line, 4);
        getline(file, line);
        timer = stoi(parsing(line, 1));
        score = stoi(parsing(line, 2));
    }
    file.close();
    file.open("PlayerBullets.txt", ios::in);
    while (getline(file, line))
    {
        playerBulletX[playerBulletCount] = stoi(parsing(line, 1));
        playerBulletY[playerBulletCount] = stoi(parsing(line, 2));
        playerBulletCount++;
    }
    file.close();
    file.open("EnemyABullets.txt", ios::in);
    while (getline(file, line))
    {
        enemyABulletX[enemyABulletCount] = stoi(parsing(line, 1));
        enemyABulletY[enemyABulletCount] = stoi(parsing(line, 2));
        enemyABulletCount++;
    }
    file.close();
    file.open("EnemyBBullets.txt", ios::in);
    while (getline(file, line))
    {
        enemyBBulletX[enemyBBulletCount] = stoi(parsing(line, 1));
        enemyBBulletY[enemyBBulletCount] = stoi(parsing(line, 2));
        enemyBBulletCount++;
    }
    file.close();
    file.open("EnemyCBullets.txt", ios::in);
    while (getline(file, line))
    {
        enemyCBulletX[enemyCBulletCount] = stoi(parsing(line, 1));
        enemyCBulletY[enemyCBulletCount] = stoi(parsing(line, 2));
        enemyCBulletCount++;
    }
    file.close();
    file.open("CommanderBullets.txt", ios::in);
    while (getline(file, line))
    {
        redFalconCommanderBulletX[redFalconCommanderBulletCount] = stoi(parsing(line, 1));
        redFalconCommanderBulletY[redFalconCommanderBulletCount] = stoi(parsing(line, 2));
...
void saveMazeToFile()
{
    //Used to write the values of all variables in text files
    fstream file;
    file.open("Maze.txt", ios::out);
    file << enemyAStanding << ",";
    file << enemyBStanding << ",";
    file << enemyCStanding << ",";
    file << redFalconCommanderStanding << ",";
    file << redFalconCommanderActive << "\n";
    file << playerHealth << ",";
    file << enemyAHealth << ",";
    file << enemyBHealth << ",";
    file << enemyCHealth << ",";
    file << redFalconCommanderHealth << "\n";
    file << remainingLives << ",";
    file << bulletsRemaining << "\n";
    file << mainPlayerX << ",";
    file << mainPlayerY << ",";
    file << enemyAX << ",";
    file << enemyAY << ",";
    ...
    file << enemyCX << ",";
    file << enemyCY << ",";
    file << redFalconCommanderX << ",";
    file << redFalconCommanderY << "\n";
    file << enemyADirection << ",";
    file << enemyBDirection << ",";
    file << enemyCDirection << ",";
    file << redFalconCommanderDirection << "\n";
    file << timer << ",";
    file << score;
    file.close();
    file.open("PlayerBullets.txt", ios::out);
    for (int i = 0; i < playerBulletCount; i++)
    {
        file << playerBulletX[i] << ",";
        file << playerBulletY[i] << "\n";
    }
    file.close();
    file.open("EnemyABullets.txt", ios::out);
    for (int i = 0; i < enemyABulletCount; i++)
    {
        file << enemyABulletX[i] << ",";
        file << enemyABulletY[i] << "\n";
    }
    file.close();
    file.open("EnemyBBullets.txt", ios::out);
    for (int i = 0; i < enemyBBulletCount; i++)
    {
        file << enemyBBulletX[i] << ",";
        file << enemyBBulletY[i] << "\n";
    }
    file.close();
    file.open("EnemyCBullets.txt", ios::out);
    for (int i = 0; i < enemyCBulletCount; i++)
    {
        file << enemyCBulletX[i] << ",";
        file << enemyCBulletY[i] << "\n";
    }
    file.close();
    file.open("CommanderBullets.txt", ios::out);
    for (int i = 0; i < redFalconCommanderBulletCount; i++)
    {
        file << redFalconCommanderBulletX[i] << ",";
        file << redFalconCommanderBulletY[i] << "\n";
    }
    file.close();
}
...
void emptyFiles()
{
    //Used to empty all of the text files
    fstream file;
    file.open("Maze.txt", ios::out);
    file << "";
    file.close();
